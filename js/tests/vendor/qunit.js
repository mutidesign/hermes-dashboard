...
(function (global$1) {
  ...
  // Authors: Philippe Rathé <prathe@gmail.com>, David Chan <david@troi.org>
	var equiv = (function () {
  	...
  	return function () {
  		var result = innerEquiv(...arguments)
   ...
   */
	...
	var TestReport = (function () {
  	function TestReport(name, suite, options) {
  		...
  		this.skipped = Boolean(options.skip)
  		this.todo = Boolean(options.todo)
	...
	Test.prototype = {
  	...
  	queue: function queue() {
  		...
  		var previousFailCount = config.storage && Number(config.storage.getItem('qunit-test-' + this.module.name + '-' + this.testName))

  		// Prioritize previously failed tests, detected from storage
  		var prioritize = config.reorder && Boolean(previousFailCount)

  		this.previousFailure = Boolean(previousFailCount)
  	...
  	pushResult: function pushResult(resultInfo) {
  		...
  		this.assertions.push({
  			result: Boolean(resultInfo.result),
  			message: resultInfo.message
  		})
  	},
    ...
    */
  	...
  	valid: function valid() {
  		...
  		return regexFilter ? this.regexFilter(Boolean(regexFilter[1]), regexFilter[2], regexFilter[3], fullName) : this.stringFilter(filter, fullName)
  	},
  ...
  // Release a processing hold, scheduling a resumption attempt if no holds remain.
	...
	function collectTests(module) {
  	...
  	while (modules.length) {
  		...
  		tests.push(...nextModule.tests)
  		modules.push(...toConsumableArray(nextModule.childModules))
  	}
  ...
  // "ignoreGlobalErrors" configuration option is enabled.
	function onError(error) {
  	...
  	if (config.current) {
  		...
  		pushFailure(...[error.message, error.fileName + ':' + error.lineNumber].concat(args))
  	} else {
	...
	(function () {
  	...
  	window.onerror = function (message, fileName, lineNumber) {
  		...
  		if (originalWindowOnError) {
  			...
  			ret = originalWindowOnError.call(...[this, message, fileName, lineNumber].concat(args))
  		}
